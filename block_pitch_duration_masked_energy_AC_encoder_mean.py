...
class ConvolutionalEncoderSmall(nn.Module):
    ...
    def __init__(self):
        ...
        self.conv2_enc = nn.Conv1d(in_channels=256, out_channels=512, 
                                    kernel_size=3, stride=2, padding=1)
        self.conv3_enc = nn.Conv1d(in_channels=512, out_channels=1024, 
                                    kernel_size=3, stride=2, padding=1)
        self.conv4_enc = nn.Conv1d(in_channels=1024, out_channels=1024, 
                                    kernel_size=3, stride=2, padding=1)
        self.conv5_enc = nn.Conv1d(in_channels=1024, out_channels=512, 
                                    kernel_size=3, stride=2, padding=1)
        self.conv6_enc = nn.Conv1d(in_channels=512, out_channels=256, 
                                    kernel_size=3, stride=2, padding=1)
        ...
        self.bn3_enc = nn.InstanceNorm1d(1024)
        self.bn4_enc = nn.InstanceNorm1d(1024)
        self.bn5_enc = nn.InstanceNorm1d(512)
...
class RatePredictorAC(nn.Module):
    def __init__(self, temp_scale=1.0): #50
        ...
        self.value_linear2 = nn.Linear(in_features=256, out_features=128)
        self.value_linear3 = nn.Linear(in_features=128, out_features=1)
    ...
    def forward(self, x, m, e): #(x, p, e)
        ...
        value = torch.mean(x, dim=-1, keepdim=False)
        value = self.silu(self.value_linear1(value))
        value = self.silu(self.value_linear2(value))
        value = self.silu(self.value_linear3(value))
        ...
        trans_out += e_proj.squeeze(dim=-1)
