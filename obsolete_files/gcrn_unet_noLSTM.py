...
# -*- coding: utf-8 -*-
"""
Created on Fri Jan  6 12:32:17 2023
...
class GCRN(nn.Module):
    def __init__(self):
        ...
        self.conv2 = GluConv2d(
            in_channels=16, out_channels=16, kernel_size=(2, 3), stride=(1, 2)
        )
        self.conv3 = GluConv2d(
            in_channels=16, out_channels=32, kernel_size=(2, 3), stride=(1, 2)
        )
        self.conv4 = GluConv2d(
            in_channels=32, out_channels=64, kernel_size=(2, 3), stride=(1, 2)
        )
        self.conv5 = GluConv2d(
            in_channels=64, out_channels=128, kernel_size=(2, 3), stride=(1, 2)
        )

        # self.glstm1 = GLSTM(groups=2)
        # self.glstm2 = GLSTM(groups=2)

        self.conv5_t_1 = GluConvTranspose2d(
            in_channels=256, out_channels=64, kernel_size=(2, 3), stride=(1, 2)
        )
        self.conv4_t_1 = GluConvTranspose2d(
            in_channels=128, out_channels=32, kernel_size=(2, 3), stride=(1, 2)
        )
        self.conv3_t_1 = GluConvTranspose2d(
            in_channels=64, out_channels=16, kernel_size=(2, 3), stride=(1, 2)
        )
        ...
        self.bn5_t_1 = nn.BatchNorm2d(64)
        self.bn4_t_1 = nn.BatchNorm2d(32)
        self.bn3_t_1 = nn.BatchNorm2d(16)
        self.bn2_t_1 = nn.BatchNorm2d(16)
        self.bn1_t_1 = nn.BatchNorm2d(1)

        self.bn5_t_2 = nn.BatchNorm2d(64)
        self.bn4_t_2 = nn.BatchNorm2d(32)
        ...
        self.inter_fc_1 = nn.Linear(in_features=128*7, out_features=128*7)
        self.inter_fc_2 = nn.Linear(in_features=128*7, out_features=128*7)
    ...
    def forward(self, x):
        ...
        e5_shape = out.shape
        # print("e5 shape: ", e5_shape)
        out = out.flatten(start_dim=-2, end_dim=-1)
        out = self.inter_fc_2(self.elu(self.inter_fc_1(out)))
        out = out.reshape(e5_shape)
        out = out.permute(0,2,1,3)
        # out = self.glstm1(out)
        # print("Intermediate output shape: ", out.shape)
