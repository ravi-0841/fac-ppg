...
# -*- coding: utf-8 -*-
"""
Created on Fri Dec 16 13:50:12 2022
...
from wav2vec_knowledge_distillation import Wav2Vec2_pretrained, Wav2Vec2_encoder
...
def prepare_directories_and_logger(output_directory, log_directory, rank):
    if rank == 0:
        ...
        logger = Wav2VecLogger(os.path.join(output_directory, log_directory))
    else:
        logger = None
    return logger
...
def load_model(hparams):
    model_train = Wav2Vec2_encoder(hparams.config_path)
    model_eval = Wav2Vec2_pretrained(hparams.config_path)
...
def validate(model_train, model_eval, criterion, valset, iteration, 
    ...
    if rank == 0:
        ...
        logger.log_validation(val_loss, model_train, x, y, y_pred, iteration)
...
def train(output_directory, log_directory, checkpoint_path, warm_start, n_gpus,
    ...
    model_train, model_eval = load_model(hparams)
    ...
    for epoch in range(epoch_offset, hparams.epochs):
        ...
        for i, batch in enumerate(train_loader):
            ...
            if (iteration % hparams.iters_per_checkpoint == 0):
                validate(model_train, model_eval, criterion, valset, iteration,
