...
from data_prep import prepare_mini_librispeech
...
class ASR(sb.Brain):
    ...
    def compute_forward(self, batch, stage):
        ...
        return predictions, encoded_signal
    ...
    def evaluate_batch(self, batch, stage):
        ...
        * ``compute_objectives()``
        ...
        out, feats = self.compute_forward(batch, stage=stage)
        loss = self.compute_objectives(out, batch, stage=stage)
        return loss.detach().cpu(), feats.detach().cpu()
    ...
    def evaluate(
        ...
        with torch.no_grad():
            for batch in tqdm(
                ...
                self.step += 1
                loss, feats = self.evaluate_batch(batch, stage=sb.Stage.TEST)
                avg_test_loss = self.update_average(loss, avg_test_loss)
                
                feats_array.append(feats.numpy())
        ...
        self.step = 0
        return avg_test_loss, feats_array
...
if __name__ == "__main__":
    ...
    hparams_file, run_opts, overrides = sb.parse_arguments(["train.yaml"])
    # print(sys.argv, hparams_file, run_opts, overrides)
    ...
    asr_brain.debug = True

    # Load best checkpoint for evaluation
    test_stats, test_feats = asr_brain.evaluate(
        test_set=datasets["test"],
        min_key="WER",
        test_loader_kwargs=hparams["test_dataloader_opts"],
    )

    joblib.dump({"encoder_features": test_feats}, "./test_feats.pkl")
