...
class BatchEnergyPitchModification():
    ...
    def __call__(self, factor_pitch, factor_energy, speech):
        ...
        for n in range(num_samples):
            pitch = batch_factor_pitch[n]
            energy = batch_factor_energy[n]
            speech = batch_speech[n]
            speech = speech.reshape((-1,))
            
            f0, sp, ap = pw.wav2world(np.asarray(speech, np.float64), 16000, 
                                      frame_period=self.frame_period)
            f0_modified = np.ascontiguousarray(pitch * f0)
            speech_modified = pw.synthesize(f0_modified, sp, ap, 16000, 
                                            frame_period=self.frame_period)
            speech_modified *= np.sqrt(energy)
